...
model_2 = tf.matmul(tf.matmul(tf.multiply(tf.multiply(X,W[:,0]),tf.transpose(W[:,0])),tf.transpose(X)),Y) #Y estimada


#print(tf.matmul(X,tf.expand_dims(get_first_col(W.value()),0)).eval())
...
cost1_1 = 20*tf.reduce_sum(cost0)
cost0_2 = tf.square(Y - model_2) #subtracts element-wise the vector and the elevates them to the 2nd power
cost1_2 = 5*tf.reduce_sum(cost0_2)
cost1 = cost1_1
#cost2 = 0
...
cost3 = 0.
""" cost3 = -(2/3)*(1/(n_ind)**2)*tf.reduce_sum(
                            tf.multiply(
                                tf.square(
                                  tf.matmul(
                                    tf.transpose(Y),
...
loss_0 = tf.add(cost1, cost2)
loss = tf.add(cost3, loss_0)  #adds all of the elements plus the error for normalization
...
final_W, loss_eu, loss1_2 = sess.run([W, loss,loss_0], {X:X_matrix, Y:Y_matrix})
print("W:")
print(final_W)
print("loss: %s"%(loss_eu/n_ind))
print("loss1_2: %s"%(loss1_2/n_ind))
#print("cost_3: %s"%(cost_3/n_ind))
